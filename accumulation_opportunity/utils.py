...
def filter_missing(df_fc_selected: pd.DataFrame, columns: List) -> pd.DataFrame:
    """Filters FC table to include only those data reported for each of the
    selected columns.
    """
...
def get_strat_result_df(
    ...
    _ = (df_strat_result.top_dollar_positions * borrowing_rate * funding_days / 360) - (
        df_strat_result.bottom_dollar_positions * repo_rate * funding_days / 360
    )

    df_strat_result["top_quantile"] = df_strat_result.top.divide(total_capital)
    df_strat_result["bottom_quantile"] = df_strat_result.bottom.divide(total_capital)
...
def get_strat_chart(
    ...
    fig.add_annotation(
        text=(
            "Notes:"
            "<br><sup>1</sup>Quantile returns based on profit divided by total "
            "capital to approximate strategy return when added together."
            "<br><sup>2</sup>Benchmark returns based on average share price "
            f"return for {df_strat_result.ntickers.max()} companies from which "
            "quantiles were selected."
        ),
        xref="paper",
        yref="paper",
        x=-0.05,
...
def get_ratio_comp_chart(
    ...
    fig.add_annotation(
        text=(
            "Line represents median ratio of quantile. Bands represent 25th "
            "and 75th percentile within strategy quantile."
        ),
        xref="paper",
        yref="paper",
        x=0.0,
...
def get_ranking_change_chart(
    ...
    fig.add_annotation(
        text=(
            "Colormap tied to rank of ratio. Tickers are sorted in ascending order at "
            "the start of the strategy<br>and then remain in the same order throughout "
            "with color updated to reflect ratio rank in subsequent periods."
        ),
        xref="paper",
        yref="paper",
        x=0.0,
